  ...
  return (
    ...
    >
      ...
      <div className="w-full p-5 flex-1 xl:min-w-620">
        ...
        <div>
          <h1 className="text-4xl font-bold brrak-words mt-3">
            {pinDetail.title}
          </h1>
        ...
        <div className="max-h-370 overflow-y-auto">
          {pinDetail?.comments?.map((item: any, index: number) => (
            <div
              className="flex gap-2 mt-5 items-center bg-white rounded-lg"
              key={index}
            >
        ...
        <div className="flex flex-wrap mt-6 gap-3">
          {userProfile && (<Link to={`/user-profile/${userProfile?._id}`}>
            <img
          ...
          <input
            ...
            type="text"
            placeholder="Add a comment bru"
            value={comment}
            onChange={(e) => setComment(e.target.value)}
          />
          {!userProfile && <p className="bg-green-200 rounded text-blue px-6 py-2 font-semibold">Login to comment brah</p>}
          <button
            type="button"
